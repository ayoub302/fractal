...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/01/20 17:56:25 by aben-sai          #+#    #+#             */
/*   Updated: 2025/01/24 12:23:32 by aben-sai         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	on_key_hook_event(int key, t_engine *engine)
{
	...
	else if ((key == KEY_L || key == KEY_L_SHIFT)
		&& engine->fractal.type == JULIA)
		engine->fractal.is_julia_lock ^= 1;
	else if (key == KEY_ZERO || key == KEY_ZERO_NUMPAD)
