...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/01/20 15:48:07 by aben-sai          #+#    #+#             */
/*   Updated: 2025/01/24 12:15:28 by aben-sai         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	reset_engine(t_engine *engine, int fractal_types)
{
	engine->fractal.type = fractal_types;
...
static void	init_mlx_objects(t_engine *engine)
{
	int	pixel_bits;
	int	line_len;
	int	endian;
	...
	engine->mlx = mlx_init();
	if (!engine->mlx)
		error_message("[MLX ERROR]: its not possible do mlx_init\n", 1);
	...
	if (!engine->window || !engine->image.img_ptr)
	{
		...
		error_message("[MLX ERROR]: its not possible handle object creation\n", \
			1);
	}
