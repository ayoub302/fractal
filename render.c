...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/01/17 16:34:12 by aben-sai          #+#    #+#             */
/*   Updated: 2025/01/24 12:29:39 by aben-sai         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	calc_fractal(t_fractal *fract, t_complex *c, int x, int y)
{
	...
	iter = 0;
	if (fract->type != JULIA)
		c->imaginary = (y / fract->zoom) + fract->offset_y;
	else if (!fract->is_julia_lock)
		c->imaginary = (fract->mouse_y / fract->zoom) + fract->offset_y;
...
void	draw_fractal(t_engine *engine)
{
	...
	while (++x < WIN_SIZE)
	{
		y = -1;
		if (fract->type != JULIA)
			c.real = (x / fract->zoom) + fract->offset_x;
		else if (!fract->is_julia_lock)
			c.real = (fract->mouse_x / fract->zoom) + fract->offset_x;
		while (++y < WIN_SIZE)
		{
